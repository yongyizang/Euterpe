...
export async function loadAlgorithm(content) {
  ...
  for (let i = 0; i < self.config.agentSettings.warmupRounds; i++) {
    ...
    const embMidiC = tf.concat([embMidi.slice([0, 0, 0], [1, 1, 150]), embMidi.slice([0, 1, 0], [1, 1, 150])], 2);
    const embCpcC = tf.concat([embCpc.slice([0, 0, 0], [1, 1, 150]), embCpc.slice([0, 1, 0], [1, 1, 150])], 2);
    const totalInp = tf.concat([embMidiC, embCpcC, embRhy], 2);
    const out = self.modelLstm.predict([totalInp, self.states1A, self.states1B, self.states2A, self.states2B]);
