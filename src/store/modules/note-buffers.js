...
const actions = {
    ...
    storeHumanQuantizedInput({commit, state, getters}, quantizedEventList) {
        ...
        const quantizedEventListOnHold = quantizedEventList.filter((event) =>
            event.type === state.noteType.NOTE_ON || event.type === state.noteType.NOTE_HOLD);
    ...
    When a note is "on", turn on pianoState and bufferState for that note,
    then set the last note played to that note.
...
const mutations = {
    ...
    initQuantBuffers(state, config) {
        ...
        const ticksPerMeasure =
            config.clockSettings.ticksPerBeat * config.clockSettings.timeSignature.numerator;
