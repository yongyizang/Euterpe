  ...
  <div class="keyboard" :style="style">
    <ul>
      <!--
        ...
        the CSS class of the key is "key.class", while the active state
        is defined by noteActive(key.name).
      ...
      <li
        ...
        :ref="key.name"
      >
        <p v-if="key.name in audioKeysMap"
            style="font-family: roboto;line-height:15px;margin-bottom:0px">
            <span style="font-size:20px;">{{ audioKeysMap[key.name] }}</span> <br />
            <span style="opacity: 0.5;font-size:12px">{{ key.name }}</span></p>
...
export default {
    ...
    created() {
        ...
        (this.offsets.octaveStart === this.offsets.octaveEnd &&
            ...
            throw new Error('The start octave must be lower than or equal' +
                ' to the end octave and the start note must be lower than the end note.',
            );
        }
    },
